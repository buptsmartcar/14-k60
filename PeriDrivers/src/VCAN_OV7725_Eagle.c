...
void ov7725_eagle_port_init()
{
    ...
    dma_portx2buff_init(CAMERA_DMA_CH, (void *)&PTD_B0_IN, (void *)ov7725_eagle_img_buff, PTB9, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);
    ...
    port_init(PTB9, ALT1 | DMA_FALLING | PULLDOWN );         //PCLK
    
    port_init(PTB2, ALT1 | IRQ_RISING  | PULLDOWN | PF);     //场中断，上拉，上降沿触发中断，带滤波
...
void ov7725_eagle_vsync(void)
{
    ...
    if(imgGetIndex==0){buffer_flag=1;}
    else if(imgGetIndex==1){buffer1_flag=1;}
    
    disable_irq(PORTB_IRQn);
    
    PORTB_ISFR = 1 <<  PT9;            //清空PCLK标志位
...
void ov7725_eagle_dma()
{
    ...
    if(imgGetIndex==0){buffer_flag=2;}
    else if(imgGetIndex==1){buffer1_flag=2;}
    DMA_IRQ_CLEAN(CAMERA_DMA_CH);           //清除通道传输中断标志位
#else
    ov7725_eagle_img_flag = IMG_FINISH ;
    DMA_IRQ_CLEAN(CAMERA_DMA_CH);           //清除通道传输中断标志位
...
/*!
*  @brief      双缓冲采集，148帧，略微不稳定
*  @since      v5.0
*/
//void ov7725_eagle_get_img_fullfps()
//{
//    
//    skipFlag=0;
//    if(!buffer_flag&&buffer1_flag==2){
//        imgGetIndex=0;ov7725_eagle_img_buff=Picbuff;
//        PORTB_ISFR = ~0;enable_irq(PORTB_IRQn);}
//    else if(!buffer1_flag&&buffer_flag==2){
//        imgGetIndex=1;ov7725_eagle_img_buff=Picbuff1;
//        PORTB_ISFR = ~0;enable_irq(PORTB_IRQn);}
//    else if(buffer_flag==2&&buffer1_flag==2){
//        imgGetIndex=0;ov7725_eagle_img_buff=Picbuff;
//        PORTB_ISFR = ~0;enable_irq(PORTB_IRQn);}
//    // else if(!buffer_flag&&!buffer1_flag){imgGetIndex=0;ov7725_eagle_img_buff=Picbuff;}
//    else{
//        skipFlag=1;
//    }//10,01,22，跳过，等待有图到来
//    
...
*       正常采集，74帧，稳定
*/
void ov7725_eagle_get_img_halffps()
{
